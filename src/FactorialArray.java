...
public class FactorialArray
{
    ...
    {
        product[0] = 1; // Initializes the first digit as 1
        
        // The remaining numbers in the array are initialized as -1
        for (int i = 1; i < product.length; i++) product[i] = -1;
        
        index = 0; // Stores the index of the end digit of the product
        
        // Calculates the factorial by muliplying by 2, 3 ... number
        for (int n = 2; n <= number; n++) multiply(n);
    }

    // Returns the contents of the array formatted as a String, excluding all leading indices containing -1
    ...
    {
        for (int j = 0; j <= index; j++) product[j] *= multiplier;
